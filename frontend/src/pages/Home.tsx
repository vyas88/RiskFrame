...
export default function Home() {
  ...
  useEffect(() => { const video = heroVideoRef.current; if (!video) return; const slowPlayback = () => { video.playbackRate = 0.75; void video.play().catch(() => undefined); }; slowPlayback(); video.addEventListener("loadedmetadata", slowPlayback); return () => video.removeEventListener("loadedmetadata", slowPlayback); }, []);
  ...
  return <Box sx={{ bgcolor: "#F4F6F8", color: "#1E2A36", minHeight: "100vh", "& *:focus-visible": { outline: "3px solid #E0A500", outlineOffset: 3 } }}>
    ...
    <Box component="main">
      <Container maxWidth="lg"><Box sx={{ ...sectionSx, position: "relative", overflow: "hidden", isolation: "isolate", minHeight: { md: 610 }, display: "flex", alignItems: "center", "&:before": { content: "''", position: "absolute", inset: 0, zIndex: -1, background: "linear-gradient(90deg, rgba(5,20,28,.91) 0%, rgba(5,20,28,.76) 53%, rgba(5,20,28,.42) 100%)" } }}><Box component="video" ref={heroVideoRef} autoPlay muted loop playsInline aria-hidden="true" sx={{ position: "absolute", inset: 0, zIndex: -2, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center" }}><source src="/riskframe-hero-bg.mp4" type="video/mp4" /></Box><Box sx={{ maxWidth: 760, color: "common.white", animation: "homeReveal 420ms ease-out both", "@keyframes homeReveal": { from: { opacity: 0, transform: "translateY(12px)" }, to: { opacity: 1, transform: "translateY(0)" } }, "@media (prefers-reduced-motion: reduce)": { animation: "none" } }}><Typography sx={{ ...eyebrowSx, color: "#A9DDE0" }}>Integrated credit risk analytics</Typography><Typography component="h1" sx={{ ...displaySx, color: "common.white", fontSize: { xs: "3rem", sm: "4rem", md: "4.8rem" }, lineHeight: .98, mt: 1.5, mb: 2.5 }}>Credit risk, made explainable.</Typography><Typography sx={{ color: "rgba(255,255,255,.82)", fontSize: { xs: "1.05rem", md: "1.2rem" }, maxWidth: 600, lineHeight: 1.65 }}>RiskFrame takes a lender's loan file all the way to an IFRS 9 provision, and explains every number on the way.</Typography><Stack direction={{ xs: "column", sm: "row" }} spacing={1.5} mt={4}><Button variant="contained" size="large" onClick={launch}>Get started</Button><Button variant="outlined" color="inherit" size="large" onClick={launchSample} sx={{ borderColor: "rgba(255,255,255,.7)", color: "common.white" }}>Use sample data</Button></Stack></Box></Box></Container>
