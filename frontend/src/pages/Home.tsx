...
export default function Home() {
  ...
  return <Box sx={{ bgcolor: "#F4F6F8", color: "#1E2A36", minHeight: "100vh", "& *:focus-visible": { outline: "3px solid #E0A500", outlineOffset: 3 } }}>
    <Box component="header" sx={{ position: "sticky", top: 0, zIndex: 10, bgcolor: "#0B1F2A", color: "common.white", borderBottom: "1px solid", borderColor: scrolled ? "rgba(255,255,255,.16)" : "transparent", transition: "border-color 180ms ease" }}><Container maxWidth="lg"><Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2} sx={{ minHeight: 76 }}><Wordmark light /><Stack direction="row" spacing={.5} alignItems="center"><Box component="nav" aria-label="Home page" sx={{ display: { xs: "none", md: "flex" } }}><Button color="inherit" onClick={() => scrollToSection("how")}>How it works</Button><Button color="inherit" onClick={() => scrollToSection("data")}>Your data</Button><Button color="inherit" component="a" href="/learn.html">See how it works</Button></Box><Button variant="contained" onClick={launch} sx={{ whiteSpace: "nowrap", px: { xs: 1.5, sm: 2.25 }, bgcolor: "#E0A500", color: "#0B1F2A", "&:hover": { bgcolor: "#F0B91F" } }}>Get started</Button></Stack></Stack></Container></Box>
    <Box component="main">
      <Container maxWidth="lg"><Box sx={{ ...sectionSx, minHeight: { md: 610 }, display: "flex", alignItems: "center", background: "linear-gradient(140deg, #F4F6F8 0%, #EAF3F3 100%)" }}><Box sx={{ maxWidth: 760, animation: "homeReveal 420ms ease-out both", "@keyframes homeReveal": { from: { opacity: 0, transform: "translateY(12px)" }, to: { opacity: 1, transform: "translateY(0)" } }, "@media (prefers-reduced-motion: reduce)": { animation: "none" } }}><Typography sx={eyebrowSx}>Integrated credit risk analytics</Typography><Typography component="h1" sx={{ ...displaySx, fontSize: { xs: "3rem", sm: "4rem", md: "4.8rem" }, lineHeight: .98, mt: 1.5, mb: 2.5 }}>Credit risk, made explainable.</Typography><Typography color="text.secondary" sx={{ fontSize: { xs: "1.05rem", md: "1.2rem" }, maxWidth: 600, lineHeight: 1.65 }}>RiskFrame takes a lender's loan file all the way to an IFRS 9 provision, and explains every number on the way.</Typography><Stack direction={{ xs: "column", sm: "row" }} spacing={1.5} mt={4}><Button variant="contained" size="large" onClick={launch}>Get started</Button><Button variant="outlined" size="large" onClick={launchSample}>Use sample data</Button></Stack></Box></Box></Container>
    ...
    <Box component="footer" sx={{ bgcolor: "#0B1F2A", color: "common.white", py: 5 }}><Container maxWidth="lg"><Box display="grid" gridTemplateColumns={{ xs: "1fr", md: "1.5fr 1fr" }} gap={4}><Box><Wordmark light /><Typography variant="body2" color="rgba(255,255,255,.72)" mt={2}>Explainable, stateless IFRS 9 credit-risk analytics.</Typography></Box><Stack alignItems={{ xs: "flex-start", md: "flex-end" }} spacing={1}><Button onClick={launch} color="inherit">Get started</Button><Button component="a" href="/learn.html" color="inherit">See how it works</Button><Button onClick={() => scrollToSection("data")} color="inherit">Your data</Button></Stack></Box></Container></Box>
  </Box>;
}
