...
export default function Report() {
  const { portfolioBody, activeLabel } = useDataSource();
  ...
  return <Panel title="RiskFrame report" subtitle="The standalone report uses the same request-scoped dashboard calculations."><Button variant="contained" onClick={download} sx={{ mb: 2 }}>Download report</Button><Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}><iframe title="RiskFrame report preview" srcDoc={html} style={{ width: "100%", minHeight: 720, border: 0 }} /></Box><Typography variant="caption">Preview uses fresh data for this request: {activeLabel}.</Typography></Panel>;
}
